...
def createPublicHolidays2018():
	...
	newYearsDay = Event(name="New Year's Day", description='The day after the new year.', country='', city='', locality='')
	newYearsDayDate.events.append(newYearsDay)

	goodFriDate = Date.query.filter_by(year=2018, month=3, date=30).one()
	goodFri = Event(name="Good Friday", description='Friday for Easter', country='', city='', locality='')
	goodFriDate.events.append(goodFri)

	easterMonDate = Date.query.filter_by(year=2018, month=4, date=2).one()
	easterMon = Event(name='Easter Monday', description='The Monday during Easter', country='', city='', locality='')
	easterMonDate.events.append(easterMon)

	earlyMayBankDate = Date.query.filter_by(year=2018, month=5, date=7).one()
	earlyMayBank = Event(name='Early May Bank Holiday', description='Bank holiday for early May', country='England', city='', locality='')
	earlyMayBankDate.events.append(earlyMayBank)

	springBankDate = Date.query.filter_by(year=2018, month=5, date=28).one()
	springBank = Event(name='Spring Bank Holiday', description='Bank holiday for Spring', country='England', city='', locality='')
	springBankDate.events.append(springBank)

	summerHolDate = Date.query.filter_by(year=2018, month=8, date=27).one()
	summerHol = Event(name='Summer Bank Holiday', description='A public holiday in Summer for banks', country='England', city='', locality='')
	summerHolDate.events.append(summerHol)

	xmasDayDate = Date.query.filter_by(year=2018, month=12, date=25).one()
	xmasDay = Event(name='Christmas Day', description='Christmas is here!', country='', city='', locality='')
	xmasDayDate.events.append(xmasDay)

	boxingDayDate = Date.query.filter_by(year=2018, month=12, date=26).one()
	boxingDay = Event(name='Boxing Day', description='The day after Christmas', country='', city='', locality='')
...
def addSportEvents2018():
	ausOpen = Event(name='Australian Open', description='The first of four tennis grand slam tournaments each year held in Australia.', country='Australia', city='Melbourne', locality='Melbourne Park')
	for x in range(15, 29):
		date = Date.query.filter_by(year=2018, month=1, date=(x)).one()
		ausOpen.dates.append(date)

	commonwealthGames = Event(name='Commonwealth Games', description='Sporting event between elite athletes of countries from the Commonwealth.', country='Australia', city='Gold Coast', locality='')
	for x in range(4, 16):
		date = Date.query.filter_by(year=2018, month=4, date=(x)).one()
		commonwealthGames.dates.append(date)

	faCupFinal = Event(name='FA Cup Final', description='The final match of the Football Association Challenge Cup', country='England', city='London', locality='')
	date = Date.query.filter_by(year=2018, month=5, date=19).one()
	faCupFinal.dates.append(date)

	uefaChampsFinal = Event(name='UEFA Champions League Final', description='The final game in the UEFA Champions League', country='Ukraine', city='Kiev', locality='NSC Olimpiyskiy Stadium')
	date = Date.query.filter_by(year=2018, month=5, date=26).one()
	uefaChampsFinal.dates.append(date)

	frenchOpen = Event(name='Roland-Garros', description='Also known as The French Open, this tennis grand slam is held on courts with clay surfaces.', country='France', city='Paris', locality='Stade Roland Garros')
	...
	fifaWorldCup = Event(name='FIFA World Cup', description="The World's most watched global sporting event held every four years.", country='Russia', city='', locality='')
	...
	nbaFinalsStart = Event(name='NBA Finals (start)', description='The start of the American National Basketball League finals series.', country='USA', city='', locality='')
	date = Date.query.filter_by(year=2018, month=5, date=31).one()
	nbaFinalsStart.dates.append(date)
	
	tourDeFrance = Event(name='Tour De France', description='The biggest multistage outdoor cycling competition held in France every year.', country='France', city='', locality='')
	for x in range(7, 30):
		date = Date.query.filter_by(year=2018, month=7, date=(x)).one()
		tourDeFrance.dates.append(date)

	wimbledon = Event(name='Wimbledon', description='The last of four tennis grand slam tournamnets each year held in Wimbledon, England.', country='England', city='London', locality='Wimbledon')
	for x in range(2, 16):
		date = Date.query.filter_by(year=2018, month=7, date=(x)).one()
		wimbledon.dates.append(date)

	melbourneCup = Event(name='Melbourne Cup', description='A major horse racing event held in Melbourne Australia', country='Australia', city='Melbourne', locality='Flemington Racecourse')
